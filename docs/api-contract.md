...
| `DELETE` | `/tontines/:id/members/:mid` | Sortie. Renvoie `resteDu` — ce que le membre doit encore sur les tours non clos —, et l'inscrit au registre avec le départ. **Le président ne peut pas sortir** : il faudrait d'abord passer la présidence |
