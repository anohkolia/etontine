...
export default defineEventHandler(async (event) => {
  ...
  const tour = await tourCourant(db, tontineId)
  const etat = tour ? (await etatDuTour(db, tour.id, membership.id)) : null
  ...
  const versement = tour ? (await etatVersement(db, tour.id, membership.userId ?? undefined)) : null

  return {
    ...tontine,
    myRole: membership.role,
    totalShares: await totalParts(db, tontineId),
    activeMembers: await membresActifs(db, tontineId),
    // Tous les montants sont calculés côté serveur (règle 2). Le client affiche.
    expectedPot: await potAttendu(db, tontineId),
    channels: await canauxDeTontine(db, tontineId),
    currentRound: tour,
    ...
    rounds: await toursDe(db, tontineId),
    publicationBlockers: tontine.status === 'draft' ? (await blocagesPublication(db, tontineId)) : [],
    ...
    startBlockers: tontine.status === 'open' ? (await blocagesDemarrage(db, tontineId, dateDuJour())) : [],
  }
})
