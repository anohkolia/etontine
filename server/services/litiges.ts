 ...
 */
export async function ouvrirContestation(db: Db, ledgerEntryId: string, acteurId: string, message: string) {
  ...
  await notifierTontine(db, ecriture.tontineId, {
...
async function tontineDe(db: Db, litige: typeof disputes.$inferSelect): Promise<string> {
  const [ecriture] = await db
 ...
 */
async function destinatairesDuFil(db: Db, tontineId: string, litige: typeof disputes.$inferSelect, sauf: string): Promise<string[]> {
 ...
 */
export async function ajouterMessage(db: Db, disputeId: string, auteurId: string, message: string) {
  const [litige] = await db.select().from(disputes).where(eq(disputes.id, disputeId)).limit(1)
  ...
  const id = randomUUID()
  await db.insert(disputeMessages).values({ id, disputeId, authorId: auteurId, body: message.trim() })

  const tontineId = await tontineDe(db, litige)
  for (const userId of await destinatairesDuFil(db, tontineId, litige, auteurId)) {
 ...
 */
export async function resoudreContestation(db: Db, disputeId: string, acteurId: string, resolution: string) {
  ...
  if (litige.openedBy !== acteurId) {
    await notifier(db, litige.openedBy, {
 ...
 */
export async function litigesDe(db: Db, tontineId: string) {
  const fils = await db
