...
test('le registre se charge page par page', async ({ page }) => {
  ...
  // d'espèces partielles — de montants tous différents, sinon la garde
  // anti-doublon les fond en une seule — et leurs confirmations d'office.
  ...
  const [premiere] = contributions
  for (let i = 0; i < 30; i++) {
    const declaration = await page.request.post(`/api/v1/contributions/${premiere!.id}/declare-cash`, {
      headers: { 'Idempotency-Key': `pagination-${i}-${Date.now()}` },
      data: { amount: 100 + i, channel: 'cash', membershipId: premiere!.membershipId },
    })
...
test('l’historique montre le tour passé et donne le reçu', async ({ page }) => {
  await inscriptionOtp(page)
  const { id } = await tontinePubliee(page, [
    { nom: 'Koffi N’Guessan', numero: numeroDeTest() },
    { nom: 'Fatou Diarra', numero: numeroDeTest() },
  ...
  // Le président déclare sa cotisation, confirmée d'office : bureau d'une seule personne.
  ...
  await expect(page.getByTestId('message-declaration')).toContainText('confirmée')

  await page.getByTestId('onglets-tontine').getByText('Historique').click()
  await page.waitForURL(/\/historique/)
  await waitForHydration(page)

  await expect(page.getByTestId('total-verse')).toContainText('25 000')
  await expect(page.getByTestId('historique-tour-1')).toContainText('c’est toi')
  await page.locator('[data-testid^="bouton-recu-"]').first().click()
  await expect(page.locator('[data-testid^="lien-recu-"]').first()).toHaveAttribute('href', /\/recu\//)
  await expect(page.getByTestId('tours-a-venir')).toContainText('2 tours à venir')
})
...
test('une déclaration en attente peut recevoir sa capture après coup', async ({ page, browser }) => {
  ...
  // Bureau à deux : la déclaration du président reste en attente.
  await page.goto(`/app/tontine/${id}/cotiser`)
  await waitForHydration(page)
  await page.locator('[data-testid^="bouton-envoyer-"]').first().click()
  await page.getByTestId('bouton-jai-envoye').click()
  ...
  const champ = page.locator('[data-testid^="champ-preuve-tardive-"]').first()
  await expect(champ).toBeAttached()
  await champ.setInputFiles({ name: 'capture.png', mimeType: 'image/png', buffer: PNG })
  await expect(page.locator('[data-testid^="preuve-jointe-"]').first()).toBeVisible({ timeout: 20_000 })

  // Le trésorier voit la capture dans sa file.
  await koffi.page.goto(`/app/tontine/${id}/confirmations`)
  await waitForHydration(koffi.page)
  await expect(koffi.page.getByTestId('file-confirmation').locator('img, a[href*="/uploads/proof/"]').first()).toBeAttached()

  await koffi.contexte.close()
})

test('le versement du pot accepte une capture', async ({ page }) => {
  await inscriptionOtp(page)
  const { id } = await tontinePubliee(page, [
    { nom: 'Koffi N’Guessan', numero: numeroDeTest() },
    { nom: 'Fatou Diarra', numero: numeroDeTest() },
  ...
  // Tout le monde a cotisé — espèces, confirmées d'office.
  const detail = await (await page.request.get(`/api/v1/tontines/${id}`)).json() as { currentRound: { id: string } }
  const { items: contributions } = await (await page.request.get(`/api/v1/rounds/${detail.currentRound.id}/contributions`)).json() as { items: Array<{ id: string, membershipId: string }> }
  for (const c of contributions) {
    await page.request.post(`/api/v1/contributions/${c.id}/declare-cash`, {
